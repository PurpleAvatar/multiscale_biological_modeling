...
We need to add the compartmentalization information in the `seed species`. Also update the initial concentrations of molecules at different states.
...
And the last thing to be done is to assign values to the parameters. Let's start with no ligand is added to the system.
