...
The figure below adds CheR and CheB to provide a complete picture of the core pathways influencing chemotaxis. To model these pathways, we will need to add quite a few molecules and reactions to our current model. In the tutorial linked below, we will expand the BioNetGen model that we built in the previous lesson, and then see if this model can replicate the adaptation behavior of *E. coli* within a changing attractant concentration.
...
When writing the code for a reaction, we need to specify each species involved. To specify an MCP here, we will need to tell the program the exact state it is at, i.e., whether bound to a ligand (2 possible states), whether bound to CheR (2 possible states), whether phosphorylated (2 possible states), and which methylation state it has (3 possible states). Therefore, there are 2 · 2 · 2 · 3 = 24 possible states for an MCP.

When writing the reaction of a ligand molecule binds to an MCP, we select the MCPs not bound to a ligand yet (1 possible states), either bound to CheR or not (2 possible states), either phosphorylated or not (2 possible states), and at any of the methylation states (3 possible states). There are 1 · 2 · 2 · 3 = 12 states that allow this binding reaction.

When writing the reaction of a CheR molecule binds to an MCP, we select the MCPs either bound to ligand or not (2 possible states), not bound to a CheR yet (1 possible state), either phosphorylated or not (2 possible states), and at any of the methylation states (3 possible states). There are 2 · 1 · 2 · 3 = 12 possible states.

When writing the reaction of an MCP at methlylation state A gets methylated by a CheR molecule, the reaction rate also depends on whether the MCP is bound to a ligand molecule. For each possible state of ligand-MCP binding, we can select an MCP either bound to CheR or not (2 possible states) and either phosphorylated or not (2 possible states). We add up the combinations of states for bound to ligand or not. So we have 1 · 1 · 2 · 1 + 1 · 1 · 2 · 1 = 4 reactions. Similarly an MCP at methlylation state B gets methylated requires us to specify 4 reactions.
